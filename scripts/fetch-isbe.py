#!/usr/bin/env python3
"""
fetch-isbe.py — Download the International Standard Bible Encyclopaedia from CrossWire.

Source: https://www.crosswire.org/ftpmirror/pub/sword/packages/rawzip/ISBE.zip
Format: zLD (compressed Lexical Dictionary), TEI XML content
Year:   1915 (public domain)
Editor: James Orr

Output:
  data/isbe/index.json      — all entries [{id, term, brief}]
  data/isbe/{slug}.json     — full entry per term {id, term, html, refs, source}

Usage:
  python3 scripts/fetch-isbe.py
  python3 scripts/fetch-isbe.py --force   # re-generate even if index exists
"""
...
OUT_DIR    = REPO_ROOT / 'data' / 'isbe'
ISBE_URL   = 'https://www.crosswire.org/ftpmirror/pub/sword/packages/rawzip/ISBE.zip'
SOURCE     = "International Standard Bible Encyclopaedia (James Orr ed., 1915)"
BRIEF_CAP  = 200  # ISBE articles are long; cap brief at 200 chars
...
def xml_to_html(xml, book_names):
    """Convert TEI entryFree XML to clean HTML with .ref anchor links."""
...
def xml_to_brief(xml):
    """Extract plain-text summary capped at BRIEF_CAP chars."""
    text = TAG_RE.sub('', xml)
    text = re.sub(r'\s+', ' ', text).strip()
    if len(text) > BRIEF_CAP:
        text = text[:BRIEF_CAP].rsplit(' ', 1)[0] + '…'
    return text
...
def main():
    ...
    print(f'Downloading {ISBE_URL} …')
    with urllib.request.urlopen(ISBE_URL) as resp:
        raw_zip = resp.read()
    print(f'  Downloaded {len(raw_zip):,} bytes')

    with zipfile.ZipFile(io.BytesIO(raw_zip)) as z:
        raw_entries = parse_sword_zld(z, 'isbe')
