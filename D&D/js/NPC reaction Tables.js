...
let npcMotivation = [ `On the run because ${searchArray(JOnTheRun)}. They are being pursued by ${enemyChooser()} because they ${deedFinder()}.`, `A vendetta against ${enemyChooser()} because ${searchArray(IVendetta)}.`, `${informationSearch()}.`, `Buying/Selling ${searchArray(KBuyingOrSelling)} at nearby location, then returning home - ${searchArray(LHomeland)}.`, `On a local quest to ${searchArray(MMinorQuest)} for the reward of ${searchArray(UTreasure)}.`, `Looking to ${questFinder()} in the service of ${searchArray(QMinorEnemy)}.`, `Looking to ${searchArray(NMajorQuest)} in the hopes of earning ${searchArray(UTreasure)}.`,]

function motivation() {
    document.getElementById("Motivation").innerHTML = searchArray(npcMotivation);
};

let areaStatus = [ `${faithOrWeave()}.`, `Home to ${searchArray(DFaction)}. Their alignment is Evil/Neutral.`, `You'll find ${searchArray(monsterEncounter)} here, and nearby is ${searchArray(UTreasure)}.`, `Benign area of NPC population or is abandoned. No treasure.`, `Home to ${searchArray(DFaction)}. Their alignment is Good/Neutral.`, `You'll find ${searchArray(monsterEncounter)} here, and nearby is ${searchArray(CIteminformation)}.`, `${hauntedOrCursed()}`]
