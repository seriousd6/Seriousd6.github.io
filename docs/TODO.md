...
- [ ] **COW synthesis loop — continue at the OT frontier** *(active focus —
  2026-07-22)*. **663/1,189 = 55.8% done** (Genesis→**2 Chronicles** + the whole
  NT complete; corpus validates clean). **2 Kings + 1 Chronicles + all 36 chapters
  of 2 Chronicles finished 2026-07-22** (genealogy + narrative fan-out profiles
  both calibrated). Frontier = **Ezra 1**; the remaining OT back-half is Ezra →
  Malachi + the poetic/wisdom books (~526 ch). Mirror the source's sparse verse
  keys exactly, EXCEPT omit out-of-range scrape keys (see loop-doc ch27 note).
  This also unblocks the Book Treatment loop (a book is treatment-eligible only
  once its synthesis is done — **2 Chronicles is now eligible**). Procedure:
