...
- [ ] **COW synthesis loop — continue at the OT frontier** *(active focus —
  2026-07-22)*. **656/1,189 = 55.2% done** (Genesis→**1 Chronicles**, **2
  Chronicles 1–29**, + the whole NT complete; corpus validates clean). **2 Kings +
  1 Chronicles + 2 Chronicles 1–29 finished 2026-07-22** (genealogy + narrative
  fan-out profiles both calibrated). Frontier = **2 Chronicles 30**; the remaining
  OT back-half is 2 Chronicles 30–36 + Ezra → Malachi + the poetic/wisdom books
  (~533 ch). **Immediate resume: 2 Chronicles 30–36** — these 7 were fanned out
  but the subagents hit a **weekly API limit (resets Jul 25, 1pm ET)** before
  writing; nothing partial landed, tree clean. Genealogy chapters synthesize from
  raw catena fine (commentators discuss the names) — mirror the source's sparse
  verse keys exactly, EXCEPT omit out-of-range scrape keys (see loop-doc ch27
  note). This also unblocks the Book Treatment loop (a book is treatment-eligible
  only once its synthesis is done). Procedure:
