...
# Build -not -path exclusions for find
$findExcludes = ($ExcludeDirs | ForEach-Object {
    "-not -path '*/$_/*' -not -name '$_'"
}) -join ' '

$findCmd = "find '$RemotePath' -type f $findExcludes -printf '%P\t%T@\t%s\n' 2>/dev/null | sort"
...
if (Test-Path $LocalPath) {
    Get-ChildItem -Path $LocalPath -Recurse -File | ForEach-Object {
        ...
        if (-not (Is-Excluded $relPath)) {
            $epochUtc = ($_.LastWriteTimeUtc - [datetime]'1970-01-01T00:00:00Z').TotalSeconds
